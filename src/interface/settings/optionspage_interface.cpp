...
bool COptionsPageInterface::CreateControls(wxWindow* parent)
{
...

	{
		...
		impl_->hidelocalpane_ = new wxCheckBox(box, nullID, _("&Hide local pane"));
		inner->Add(impl_->swap_);
		inner->Add(impl_->hidelocalpane_);
		...
		impl_->hidelocalpane_->Bind(wxEVT_CHECKBOX, &COptionsPageInterface::OnLayoutChange, this);
	}
...
bool COptionsPageInterface::SavePage()
{
	...
	m_pOptions->set(OPTION_LOCALPANE_HIDE, impl_->hidelocalpane_->GetValue());
